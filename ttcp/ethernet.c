...
ethernet_dump(struct netdev *dev, uint8_t *frame, size_t flen) {
    ...
    fprintf(stderr, "  src: %s\n", ethernet_addr_ntop(dev->src, addr, sizeof(addr)));
    fprintf(stderr, "  dst: %s\n", ethernet_addr_ntop(dev->dst, addr, sizeof(addr)));
    fprintf(stderr, " type: 0x%4x (%s)\n", ntoh16(hdr->type), ethernet_addr_ntoa(hdr->type));
...
int ethernet_open(struct netdev *dev, int opt) {
    ...
    memcmp(dev->broadcast, ETHERNET_ADDR_BROADCAST, ETHERNET_ADDR_LEN);
    return 0;
}
