...
void ethernet_recv(uint8_t *buf, ssize_t len) {
...
ssize_t ethernet_send(uint16_t type, const uint8_t *buf, size_t len, const ethernet_addr_t *dst) {
    uint8_t frame[ETHERNET_FRAME_SIZE_MAX];
    struct ethernet_hdr *hdr;
    size_t frame_size;

    if (!buf || len > ETHERNET_FRAME_SIZE_MAX || !dst) {
        return -1;
    }
