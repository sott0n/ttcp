...
/* TIME_EXEEDED */
#define ICMP_CODE_EXEEDED_TTL 0
#define ICMP_CODE_EXEEDED_FRAGMENT 1

#define ICMP_COPY_LEN(x) ((((x)->vhl & 0x0f) << 2) + 8)

extern int icmp_tx(struct netif *iface, uint8_t type, uint8_t code, uint32_t values, uint8_t *data, size_t len, ip_addr_t *dst);
