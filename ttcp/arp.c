...
struct arp_ethernet {
    struct arp_hdr hdr;
    uint8_t sha[ETHERNET_ADDR_LEN];     // Source hardware address (= MAC address)
    ip_addr_t spa;                      // Source internetwork address
    uint8_t tha[ETHERNET_ADDR_LEN];     // Destination hardware address (= MAC address)
...
arp_table_update(struct netdev *dev, const ip_addr_t *pa, const uint8_t *ha) {
    ...
    if (entry->data) {
        if (entry->netif->def != dev) {
            /* waring: receive response from unintended device */
            dev = entry->netif->dev;
        }
...
arp_send_request(struct netif *netif, const ip_addr_t *pa) {
    ...
    memcpy(request.tha, 0, ETHERNET_ADDR_LEN);
    request.tpa = *tpa;
#ifdef DEBUG
